...
class BookmarkControllerTest {
    ...
    @ParameterizedTest
    @CsvSource({
            "1,10,15,2,1,true,false,true,false",
            "2,10,15,2,2,false,true,false,true"
    })
    void shouldGetBookmarks(int pageNo, int size, int totalElements, int totalPages, int currentPage,
                            boolean isFirst, boolean isLast,
                            boolean hasNext, boolean hasPrevious) throws Exception {
        mvc.perform(get("/api/bookmarks?page="+pageNo+"&size="+size))
